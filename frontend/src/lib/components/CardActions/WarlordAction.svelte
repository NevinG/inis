<script lang="ts">
	...
	export async function selectTile(tileId: string) {
		...
		if (tile!.selected) {
			...
			if (
				(tile?.clans[restrictedGameState.playerId] ?? 0 > 0) &&
				(Object.values(tile?.clans ?? {}).filter(x => x > 0).length > 1)
			) { tile!.selected = true; }
		}
...
>
	<span>Choose territory to start a clash in.</span>&nbsp;
	<button
		disabled={!selectedTile}
		on:click={async () => {
			socket.send(
				JSON.stringify(await GameActionFactory.warlordActionCard(gameId, selectedTile.tileId))
			);
		}}
