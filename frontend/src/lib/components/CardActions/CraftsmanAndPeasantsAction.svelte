...
<div style:width="100%" style:height="65%">
	<GameMap {restrictedGameState} selectTile={async () => {}} {addedClans} let:tile>
		{#if restrictedGameState.playerId in tile.clans && tile.clans[restrictedGameState.playerId] > 0 && tile.citadels > 0}
			<div>
				{#if (addedClans.find((addClan) => addClan.territory == tile.tileId)?.numClans ?? 0) < tile.citadels}
					<button
						on:click={() => {
							addClan(tile);
						}}>+</button
...
>
	...
	<button
		on:click={async () => {
			socket.send(
				JSON.stringify(await GameActionFactory.craftsmenAndPeasantsActionCard(gameId, addedClans))
			);
		}}
