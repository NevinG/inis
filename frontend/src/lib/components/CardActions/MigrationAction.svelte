<script lang="ts">
	...
	function moveTroop(tile: GameTile & { selected: boolean }) {
		const move = clanMoves.find((move) => move.to == tile.tileId);
		if (move) {
			move.numClans++;
		} else {
			clanMoves.push({ from: targetTile, to: tile.tileId, numClans: 1 });
		}
		clanMoves = clanMoves;
	}
	...
	function takeTroopBack(tile: GameTile & { selected: boolean }) {
		...
		if (move) {
			move.numClans--;
			if (move.numClans == 0) {
				clanMoves = clanMoves.filter((move) => move.to != tile.tileId);
			}
		}
		clanMoves = clanMoves;
	}
	...
	<GameMap {restrictedGameState} {selectTile} {clanMoves} let:tile>
		{#if actionStep == 1 && tile.tileId != targetTile && tilesAdjacent( tile, restrictedGameState.tiles.find((tile) => tile.tileId == targetTile) )}
			{#if (restrictedGameState.tiles.find((tile) => tile.tileId == targetTile)?.clans[restrictedGameState.playerId] ?? 0) - clanMoves.reduce((totalFrom, move) => (move.from == targetTile ? move.numClans + totalFrom : totalFrom), 0) > 0}
				...
				>
			{/if}
			{#if clanMoves.find((move) => move.to == tile.tileId)?.numClans ?? 0 > 0}
				<button
					on:click={() => {
						takeTroopBack(tile);
					}}>Move Back</button
...
>
	...
	{#if actionStep == 1}
		...
		<button
			...
			on:click={async () => {
				socket.send(JSON.stringify(await GameActionFactory.migrationActionCard(gameId, clanMoves)));
			}}
